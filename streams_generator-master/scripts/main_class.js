...
function deleteView(type, id) {
    ...
    if (type == `viewCurrent`) {
        streamViews.removeChild(document.getElementById(`currentView-${id}`));
    }
    if (type == `viewHistory`) {
        streamViews.removeChild(document.getElementById(`historyView-${id}`));
    }
}
...
function deleteRangesBox(streamId, id) {
    console.log(streamId, id);
    let target = document.getElementById(`rangesDiv-${streamId}`);
    target.removeChild(document.getElementById(`range-${streamId}-${id}`));
}
...
function deleteYlineField(id) {
    let target = document.getElementById(`ylineBox-${id}`);
    target.removeChild(document.getElementById(`yline-${id}`));
}
    class infCollector{
    collectInformation() {
        let streams = [];
    ...
    constructor(inf) {
        ...
        for(let streamNumber = 0; streamNumber < streamsCounter; streamNumber++) {
            ...
            let ranges = false;
            if (stream.rangesNumber) {
                ranges = this.buildRanges(stream.ranges);
            }
            ...
            let yline = false;
            if (stream.isYline) {
                yline = this.buildYline(stream.yline);
            }
     ...
     postXml() {
        ...
        let target = document.getElementById(`xml`);
        target = createDiv(target, `xml-info`, `code`);
        createPre(target, this.xml);
    }
    ...
    function rangeFieldEvent(event) {
        ...
        if (rangesCounter[id] < value) {
            ...
            for(rangesCounter[id]; rangesCounter[id] > value; rangesCounter[id]--) {
                deleteRangesBox(id, rangesCounter[id] - 1);
            }
        }
    }
    ...
    function ylineCheck(event) {
        ...
        else deleteYlineField(id); 
    }
    
    function build(event) {
        if (streamsCounter === 0) console.log(`nothing to do`);
        else new createXml(new infCollector()).postXml();
    }
